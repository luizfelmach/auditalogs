...
pub async fn get_hash_storage(
    State(state): State<AppState>, Path(batch_id): Path<String>, Extension(cache): Extension<CacheBatchResponse>,
    ...
    let response = BatchResponse { id: batch_id.clone(), count: batch.count, hash: batch.hash.to_hex() };
...
pub async fn search_docs(State(state): State<AppState>, Json(query): Json<QueryExpr>) -> Result<Json<Vec<Storable>>> {
    let results = state.storage.search(query).await.context("An error ocurrued when processing query")?;
    Ok(Json(results))
}
